...
def clingo_json(files, tmax, radius, extra, hard_timeout):
    ...
    t0 = time.time()
    try:
        out = subprocess.run(cmd, capture_output=True, text=True, timeout=hard_timeout).stdout
...
def run_one(name, files, tmax, radius=None, time_limit=300):
    ...
    if d is None:
        ...
        g, _ = clingo_json(files, tmax, radius, ["--solve-limit=0"], hard_timeout=120)
        ...
        return {
            ...
            "ground_s": (g or {}).get("Time", {}).get("Total", 0) if g else None,
            "best_model_s": None,
            "solve_s": wall,
    ...
    for call in d.get("Call", []):
        for w in call.get("Witnesses", []):
            if "Costs" in w:
                cost = w["Costs"]
    if res == "OPTIMUM FOUND" and cost is not None:
...
def main():
    ...
    def go(*a, **kw):
        ...
        rows.append(row)
        print(
            f"  done: {row['name']} T={row['tmax']} r={row['radius']} "
            f"-> {row['result']} ({row['solve_s']:.2f}s)",
            flush=True,
        )

    # 1. horizon scaling, trivial 1-atom instance (phase-1 core)
    for t in (10, 20, 40, 80):
        go("trivial (1 atom)", [*core1, "asp/trivial_instance.lp"], t, time_limit=tl)

    # 2. radius scaling, trivial instance at T=20
    for r in (2, 3, 4):
        go("trivial (1 atom)", [*core1, "asp/trivial_instance.lp"], 20, radius=r, time_limit=tl)

    # 3. product size scaling
    go("bond 2 atoms (v1 core)", [*core1, "asp/bond_instance.lp"], 16, time_limit=tl)
    go("rigid dimer move (v2)", [*core2, "asp/rigid_instance.lp"], 10, time_limit=tl)
    go("STABILIZED WATER (true spec)", [*core2, "asp/stabilized_water.lp"], 10, time_limit=tl)
    go(
        "STABILIZED WATER, free layout",
        [*core2, "asp/layout.lp", "asp/stabilized_water_free.lp"],
        10,
        time_limit=tl,
    )
    go("STABILIZED WATER, slack T=20", [*core2, "asp/stabilized_water.lp"], 20, time_limit=tl)
    go("STABILIZED WATER, slack T=40", [*core2, "asp/stabilized_water.lp"], 40, time_limit=tl)
    go("SW bent 3-atom, 1 arm", [*core2, "asp/sw_bent.lp"], 25, time_limit=tl)
    go("SW bent 3-atom, slack T=30", [*core2, "asp/sw_bent.lp"], 30, time_limit=tl)
    go("SW linear 3-atom, 2 arms", [*core2, "asp/sw_linear.lp"], 36, time_limit=tl)

    # 4. UNSAT probes
    go("SW bent, T=24 (<optimum)", [*core2, "asp/sw_bent.lp"], 24, time_limit=tl)
    go("SW linear 1 arm (impossible)", [*core2, "asp/sw_linear_1arm.lp"], 25, time_limit=tl)

    print()
    print(
        "| instance | t_max | radius | ground rules | ground atoms | "
