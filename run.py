...
def solve(files, tmax=None, time_limit=None, extra_args=(), consts=()):
    ...
    best = {"symbols": None, "cost": None, "model_time": None}
...
def parse_model(symbols):
    do, orient, at, holding, bonds, types = {}, {}, {}, {}, {}, {}
    complete = set()
    for s in symbols:
        a = s.arguments
        if s.name == "do":
            ...
            at.setdefault(a[3].number, {})[str(a[0])] = (a[1].number,
                                                         a[2].number)
        elif s.name == "holding":
            if len(a) == 2:
...
def print_trace(symbols):
    ...
    for t in range(tmax + 1):
        ...
        bond_str = "".join(f"  bond({x},{y})"
                           for x, y in sorted(bonds.get(t, ())))
        arms = " ".join(f"{m}:d{d}" for m, d in sorted(orient.get(t,
                                                                  {}).items()))
