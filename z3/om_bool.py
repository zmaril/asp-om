...
class BoolEncoder:
    ...
    def _build(self):
        ...
        grip_hex = [self.hidx.get((base[0] + L * dq, base[1] + L * dr))
                    for (dq, dr) in DIRS]
        ...
        self.b = {p: [Bool(f"b_{p[0]}_{p[1]}_{t}") for t in range(T + 1)]
                  for p in self.pairs} if need_bonds else {}
        self.salt = None
        if inst["glyph_calcs"]:
            for a in inst["atoms"]:
                assert a.get("type", "salt") in ("salt", "water"), \
                    "bool variant tracks only salt/elemental"
            self.salt = [[Bool(f"salt_{i}_{t}") for t in range(T + 1)]
                         for i in range(n)]
        ...
        for i, atom in enumerate(inst["atoms"]):
            ...
            if self.salt is not None:
                ...
                C.append(self.salt[i][0] if s0
                         else z3.Not(self.salt[i][0]))
        for d in range(6):
        ...
        def at_grip(i, t):
            return z3.Or([z3.And(self.orient[d][t],
                          ...
                          for d in range(6) if grip_hex[d] is not None])
        ...
        for t in range(T + 1):
            ...
            for p in self.pairs:
                if not self.b:
                    break
                prev = z3.BoolVal(p in init_bond_set) if t == 0 \
                    else self.b[p][t - 1]
        ...
        for t in range(T):
            ...
            if self.v2:
                ...
                for i in range(n):
                    ...
                    for h in range(H):
                        via_cw = z3.And(cw, ci, self.at[i][cw_src[h]][t]) \
                            if cw_src[h] is not None else z3.BoolVal(False)
                        via_ccw = z3.And(ccw, ci, self.at[i][ccw_src[h]][t]) \
                            if ccw_src[h] is not None else z3.BoolVal(False)
                        C.append(self.at[i][h][t + 1] == z3.Or(
                            via_cw, via_ccw,
                            z3.And(z3.Not(moved), self.at[i][h][t])))
            else:
            ...
            if self.salt is not None:
                calc_h = [self.hidx[g] for g in inst["glyph_calcs"]
                          if g in self.hidx]
                for i in range(n):
                    on_calc = z3.Or([self.at[i][h][t] for h in calc_h]) \
                        if calc_h else z3.BoolVal(False)
                    C.append(self.salt[i][t + 1] ==
                             z3.Or(self.salt[i][t], on_calc))
        ...
        if slots:
            ...
            for perm in itertools.permutations(range(n), len(slots)):
                ...
                for si, (pos, ty) in enumerate(slots):
                    i = perm[si]
                    terms += [self.at[i][self.hidx[pos]][T],
                              z3.Not(self.held[i][T])]
                    if self.salt is not None:
                        terms.append(self.salt[i][T] if ty == "salt"
                                     else z3.Not(self.salt[i][T]))
