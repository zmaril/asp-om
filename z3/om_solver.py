...
INSTANCES = {
    ...
    "trivial": dict(
        semantics="v1", t_max=10, radius=2,
        arms=[dict(name="m1", base=(0, 0), length=1, init_orient=0)],
        atoms=[dict(name="a1", pos=(1, 0), type="salt")],
        init_bonds=[],
        glyph_bonds=[],
        glyph_calcs=[],
        products=[("a1", (-1, 0))],
    ...
    "bond": dict(
        semantics="v1", t_max=16, radius=2,
        arms=[dict(name="m1", base=(0, 0), length=1, init_orient=0)],
        atoms=[dict(name="a1", pos=(1, 0), type="salt"),
               dict(name="a2", pos=(0, 1), type="salt")],
        init_bonds=[],
        glyph_bonds=[((-1, 0), (0, -1))],
        glyph_calcs=[],
    ...
    "rigid": dict(
        semantics="v2", t_max=10, radius=2,
        arms=[dict(name="m1", base=(0, 0), length=1, init_orient=0)],
        atoms=[dict(name="a1", pos=(1, 0), type="salt"),
               dict(name="a2", pos=(2, 0), type="salt")],
        init_bonds=[("a1", "a2")],
        glyph_bonds=[],
        glyph_calcs=[],
...
class Encoder:
    ...
    def at_grip(self, i, m, t):
        return z3.And(self.q[i][t] == self.gq[m][t],
    ...
    def _build(self):
        ...
        for gi, _ in enumerate(inst["glyph_bonds"]):
            self.glyphs.append(tuple(Int(f"gb{gi}_{k}") for k in
                                     ("q1", "r1", "q2", "r2")))
        self.calcs = []
        ...
        for m, arm in enumerate(inst["arms"]):
            if self.free:
                ...
                C.append(z3.And(self.orient[m][0] >= 0,
                                self.orient[m][0] <= 5))
            else:
        ...
        if self.free:
            for m1 in range(n_arm):
                for m2 in range(m1 + 1, n_arm):
                    C.append(z3.Or(self.baseq[m1] != self.baseq[m2],
                                   self.baser[m1] != self.baser[m2]))
        for gi, ((q1, r1), (q2, r2)) in enumerate(inst["glyph_bonds"]):
            g = self.glyphs[gi]
            if self.free:
                C.append(self.on_board(g[0], g[1]))
                C.append(self.on_board(g[2], g[3]))
                C.append(z3.Or([z3.And(g[2] == g[0] + dq, g[3] == g[1] + dr)
                                for (dq, dr) in DIRS]))
            else:
        ...
        init_bond_set = {(min(name2idx[x], name2idx[y]),
                          max(name2idx[x], name2idx[y]))
        ...
        def form(i, j, t):
            ...
            for g in self.glyphs:
                ...
                disj.append(z3.Or(z3.And(a_on_1, b_on_2),
                                  z3.And(a_on_2, b_on_1)))
        ...
        for t in range(T + 1):
            for m, arm in enumerate(inst["arms"]):
                ...
                C.append(self.gq[m][t] == self.baseq[m]
                         + L * dir_component(self.orient[m][t],
                                             [d[0] for d in DIRS]))
            ...
            for i in range(n_atom):
                ...
                if v2:  # arm bases block hexes (core2 only)
                    for m in range(n_arm):
                        C.append(z3.Or(self.q[i][t] != self.baseq[m],
                                       self.r[i][t] != self.baser[m]))
            for (i, j) in self.pairs:  # no two atoms share a hex
                C.append(z3.Or(self.q[i][t] != self.q[j][t],
            ...
            for (i, j) in self.pairs:
                if not self.b:
                    break
                base = z3.BoolVal((i, j) in init_bond_set) if t == 0 \
                    else self.b[(i, j)][t - 1]
        ...
        for t in range(T):
            ...
            for m in range(n_arm):
                ...
                grabbable = z3.Or([self.at_grip(i, m, t)
                                   for i in range(n_atom)])
                ...
                if v2 and n_arm > 1:
                    ...
                    for i in range(n_atom):
                        C.append(z3.Implies(
                            ...
                            z3.Not(self.held_any(i, t))))
                    # a rotation may not tear an atom from another arm
                    for i in range(n_atom):
                        others = z3.Or([self.held[m2][i][t]
                                        for m2 in range(n_arm) if m2 != m])
                        C.append(z3.Implies(
            ...
            if inst["forbid_rot_holding_bonded"] and self.b:
                for m in range(n_arm):
                    rot = z3.Or(self.is_act(t, m, "rot_cw"),
                                self.is_act(t, m, "rot_ccw"))
                    for i in range(n_atom):
                        bondpart = z3.Or([self.bond_var(i, j, t)
                                          for j in range(n_atom) if j != i])
                        C.append(z3.Implies(
                            rot, z3.Not(z3.And(self.held[m][i][t],
                                               bondpart))))
            # position updates
            for i in range(n_atom):
                if v2:
                    ...
                    for m in range(n_arm):
                        ...
                        cw = z3.And(self.is_act(t, m, "rot_cw"),
                                    self.comp[m][i][t])
                        ccw = z3.And(self.is_act(t, m, "rot_ccw"),
                                     self.comp[m][i][t])
            ...
            if self.typ is not None:
                for i in range(n_atom):
                    on_calc = z3.Or([z3.And(self.q[i][t] == g[0],
                                            self.r[i][t] == g[1])
                                     for g in self.calcs])
                    elemental = z3.Or([self.typ[i][t] == e
                                       for e in ELEMENTAL])
                    C.append(self.typ[i][t + 1] == z3.If(
                        z3.And(on_calc, elemental),
    ...
    def goal(self, h):
        ...
        for (name, (tq, tr)) in inst["products"]:
            i = name2idx[name]
            conj += [self.q[i][h] == tq, self.r[i][h] == tr,
                     z3.Not(self.held_any(i, h))]
        if inst["require_end_bond"]:
            conj.append(z3.Or([self.b[p][h] for p in self.pairs
                               if p in self.b]))
...
def solve_optimize(enc):
    ...
    if res == z3.sat:
        mdl = opt.model()
        return dict(status="sat", cost=mdl.eval(enc.cost).as_long(),
                    model=mdl, time=wall, horizon=enc.T)
    return dict(status=str(res), cost=None, model=None, time=wall,
                horizon=enc.T)
...
def solve_ramp_cost(enc):
    ...
    for k in range(enc.T + 1):
        ...
        if s.check(guard) == z3.sat:
            ...
            return dict(status="sat", cost=mdl.eval(enc.cost).as_long(),
                        model=mdl, time=wall, horizon=enc.T)
    return dict(status="unsat", cost=None, model=None,
                time=time.perf_counter() - t0, horizon=enc.T)
...
def solve_descend_cost(enc):
    ...
    wall = time.perf_counter() - t0
    if best is None:
        return dict(status="unsat", cost=None, model=None, time=wall,
                    horizon=enc.T)
    return dict(status="sat", cost=best[0], model=best[1], time=wall,
                horizon=enc.T)
...
def solve_ramp_horizon(enc):
    ...
    for h in range(1, enc.T + 1):
        ...
        if s.check(guard) == z3.sat:
            wall = time.perf_counter() - t0
            mdl = s.model()
            cost = sum(1 for t in range(h)
                       if mdl.eval(enc.act[t]).as_long() != 0)
            return dict(status="sat", cost=cost, model=mdl, time=wall,
                        horizon=h)
    return dict(status="unsat", cost=None, model=None,
                time=time.perf_counter() - t0, horizon=None)
...
def extract_solution(enc, result):
    ...
    layout = dict(
        arms=[dict(name=arm["name"],
                   base=[ev(enc.baseq[m]).as_long(),
                         ev(enc.baser[m]).as_long()],
                   length=arm["length"],
                   init_orient=ev(enc.orient[m][0]).as_long())
              for m, arm in enumerate(inst["arms"])],
        glyph_bonds=[[ev(g[0]).as_long(), ev(g[1]).as_long(),
                      ev(g[2]).as_long(), ev(g[3]).as_long()]
                     for g in enc.glyphs],
        glyph_calcs=[[ev(g[0]).as_long(), ev(g[1]).as_long()]
                     for g in enc.calcs],
    )
    actions = []
    for t in range(h):
        aname, arm = decode_action(ev(enc.act[t]).as_long(), inst["arms"])
        actions.append(dict(t=t, action=aname, arm=arm))
    traj = {}
    for i, name in enumerate(enc.atom_names):
        traj[name] = [[ev(enc.q[i][t]).as_long(), ev(enc.r[i][t]).as_long()]
                      for t in range(h + 1)]
    orient = {arm["name"]: [ev(enc.orient[m][t]).as_long()
                            for t in range(h + 1)]
              for m, arm in enumerate(inst["arms"])}
    held = {}
    for i, name in enumerate(enc.atom_names):
        held[name] = [next((inst["arms"][m]["name"] for m in range(enc.n_arm)
                            if z3.is_true(ev(enc.held[m][i][t]))), None)
                      for t in range(h + 1)]
    bonds = []
...
def print_plan(sol, name=""):
    ...
    for arm in lay["arms"]:
        print(f"  arm {arm['name']}: base=({arm['base'][0]},{arm['base'][1]})"
