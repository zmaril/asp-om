...
def horizon_iteration(inst, mode, backend, timeout, cap, log, start=1):
    ...
    for T in range(start, cap + 1):
        ...
        sat, model, solve_t = solve(backend, enc.cnf, assumptions, timeout=timeout)
        rows.append(
            {
                "case": inst.name,
                "mode": mode,
        ...
        if sat is None:
            ...
            if consecutive_timeouts >= 2:
                log(f"    abandoning series after {consecutive_timeouts} consecutive timeouts")
...
def minimize_instructions(inst, mode, backend, t_max, timeout, log):
    ...
    if sat is None:
        ...
        log(f"  {inst.name}/{mode} opt@T={t_max} [{backend}]: first solve TIMEOUT after {st:.1f}s")
        return {
            "case": inst.name,
            "mode": mode,
            "t_max": t_max,
    ...
    def cost_of(m):
        ...
        return sum(1 for lit in lits if (lit > 0 and lit in tr) or (lit < 0 and -lit not in tr))
    ...
    while k >= 0:
        ...
        sat, model, st = solve(backend, bounded, assumptions, timeout=timeout)
        if sat is None:
            proved = False
    ...
    log(
        f"  {inst.name}/{mode} opt@T={t_max} [{backend}]: "
...
def main():
    ...
    all_rows = []
    makespans: dict[tuple[str, str], int] = {}
    plans = {}
            ...
            for backend in BACKENDS:
                ...
                if satT is not None:
                    ...
                    if prev is not None and prev != satT:
                        raise SystemExit(
                            f"backend disagreement on min makespan {key}: {prev} vs {satT}"
                        )
                    makespans[key] = satT
    ...
    sw = SW_CASES["sw"]
    for mode in MODES:
        opt_results.append(minimize_instructions(sw, mode, "cadical195", 10, args.timeout, log))
    # slack horizons (clingo's wait-padding cliff: fixed T=20 was >300s
    # for clingo without its nowait symmetry breaker)
    for t_max in (16, 20):
        opt_results.append(
            minimize_instructions(sw, "fixed", "cadical195", t_max, args.timeout, log)
        )
    opt_results.append(minimize_instructions(sw, "free", "cadical195", 16, args.timeout, log))
    ...
    if not args.skip_scaling:
        for radius in (3, 4, 5):
            ...
            for backend in ("cadical195", "kissat"):
                rows, satT, plan = horizon_iteration(inst, "free", backend, args.timeout, 12, log)
                scaling_rows.extend(rows)
                if satT is not None and backend == "cadical195":
                    plans[(inst.name, "free")] = plan
                    makespans[(inst.name, "free")] = satT

    md = render_markdown(all_rows, makespans, plans, opt_results, scaling_rows, args.timeout)
    out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "results-stabilized-water.md")
            ...
            json.dump(
                {
                    ...
                    "makespans": {f"{k[0]}/{k[1]}": v for k, v in makespans.items()},
                    "opt": [{kk: vv for kk, vv in o.items() if kk != "plan"} for o in opt_results],
                },
                f,
                indent=1,
            )
...
def render_markdown(rows, makespans, plans, opt_results, scaling_rows, timeout):
    lines = []
    lines.append("# SAT arm — Stabilized Water (Opus Magnum campaign puzzle P007)\n")
    lines.append(
    ...
    lines.append("## Horizon iteration (instance x mode x backend x T)\n")
    lines.append("| instance | mode | backend | T | vars | clauses | encode s | solve s | result |")
    lines.append("|---|---|---|---|---|---|---|---|---|")
    for r in rows:
        lines.append(
    ...
    lines.append("|---|---|---|---|")
    clingo_ms = {
        ("sw", "fixed"): "10 (optimum 10 instr, 0.07 s)",
        ("sw", "free"): "9 (SAT T=9 1.3 s; UNSAT T=8 114 s)",
    }
    for k, T in sorted(makespans.items()):
        lines.append(f"| {k[0]} | {k[1]} | {T} | {clingo_ms.get(k, '—')} |")
    lines.append("")

    lines.append("## Instruction-count minimization (clingo `#minimize` parity)\n")
    lines.append(
    ...
    lines.append("| instance | mode | t_max | best (non-wait) | proved optimal | total s | note |")
    lines.append("|---|---|---|---|---|---|---|")
    ...
    if scaling_rows:
        lines.append("## Free-layout board-radius scaling (tractability probe)\n")
        lines.append(
            ...
            "clauses range over base x hex).\n"
        )
        lines.append("| instance | backend | T | vars | clauses | encode s | solve s | result |")
        lines.append("|---|---|---|---|---|---|---|---|")
    ...
    for k in sorted(plans):
        ...
        inst = SW_CASES.get(name) or sw_scaled(int(name.split("-r")[1]))
