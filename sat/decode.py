...
try:
    from instances import DIRS, ACTIONS
except ImportError:
    from sat.instances import DIRS, ACTIONS
...
def decode_model(enc, model):
    ...
    plan["length"] = next(l for l in enc.lengths if tv("alen", l))
    ...
    plan["instructions"] = [
        next(a for a in ACTIONS if tv("do", a, t)) for t in range(T)]
    plan["orient"] = [
        next(d for d in range(6) if tv("orient", d, t)) for t in range(T + 1)]
    plan["gripper"] = [
        next(h for h in enc.hexes if tv("grip", h, t)) for t in range(T + 1)]
    plan["traj"] = {
        x: [next(h for h in enc.hexes if tv("at", x, h, t))
            for t in range(T + 1)]
        for x in inst.atoms}
    plan["hold"] = [
        next((x for x in inst.atoms if tv("hold", x, t)), None)
        for t in range(T + 1)]
    plan["bonds"] = [
...
def format_plan(plan, inst):
    ...
    lines = []
    lines.append(f"layout: base={plan['base']} length={plan['length']} "
                 f"init_orient={plan['orient0']}"
                 + (f" glyph_bond={plan['glyph'][0]}-{plan['glyph'][1]}"
                    if plan["glyph"] else ""))
    ...
    for t in range(T + 1):
        atoms = "  ".join(
            f"{x}@{plan['traj'][x][t]}" for x in inst.atoms)
        held = f" holding={plan['hold'][t]}" if plan["hold"][t] else ""
        bonds = ""
        if plan["bonds"][t]:
            bonds = " bonds=" + ",".join(
                "-".join(sorted(p)) for p in sorted(
                    plan["bonds"][t], key=sorted))
        state = (f"  t={t:2d} grip={plan['gripper'][t]} "
