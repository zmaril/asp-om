...
def decode_model2(enc, model):
    ...
    plan["spawns"] = [
        next(h for h in enc.hexes if tv("spawn", i, h)) for i in range(1, len(inst.pools) + 1)
    ]
    ...
    plan["instructions"] = [next(a for a in ACTIONS2 if tv("do", a, t)) for t in range(T)]
    ...
    plan["exists"] = {x: [tv("ex", x, t) for t in range(T + 1)] for x in X}
    plan["traj"] = {
        x: [next((h for h in enc.hexes if tv("at", x, h, t)), None) for t in range(T + 1)]
        for x in X
    }
    plan["salt"] = {x: [tv("salt", x, t) for t in range(T + 1)] for x in X}
    plan["hold"] = [next((x for x in X if tv("hold", x, t)), None) for t in range(T + 1)]
...
def format_plan2(plan, inst):
    ...
    for t in range(T + 1):
        atoms = "  ".join(
            f"{_aname(x)}[{'salt' if plan['salt'][x][t] else 'water'}]@{plan['traj'][x][t]}"
            for x in inst.atoms
            if plan["exists"][x][t]
        )
        held = f" holding={_aname(plan['hold'][t])}" if plan["hold"][t] else ""
        bonds = ""
        if plan["bonds"][t]:
            bonds = " bonds=" + ",".join(
                "-".join(_aname(a) for a in sorted(p)) for p in sorted(plan["bonds"][t], key=sorted)
            )
        lines.append(
            f"  t={t:2d} grip={plan['gripper'][t]} D={plan['orient'][t]}  {atoms}{held}{bonds}"
        )
        if t < T:
