...
def parse_dzn(path):
    ...
    for stmt in src.split(";"):
        ...
        if val in ("true", "false"):
            ...
            out[name] = [int(x) for x in inner_m.group(1).split(",") if x.strip()]
        elif val.startswith("["):
...
def bonded_pairs(sol, t, n_atoms):
    return sorted(
        (a, b) for a in range(n_atoms) for b in range(n_atoms) if a < b and sol["bonded"][t][a][b]
    )
...
def print_trace(sol):
    ...
    for t in range(T + 1):
        arms = "  ".join(
            f"arm{m + 1} dir={sol['ori'][t][m]} gripper=({sol['gq'][t][m]},{sol['gr'][t][m]})"
            for m in range(nM)
        )
        atoms = "  ".join(
        ...
        bonds = "".join(f"  bond(a{a + 1},a{b + 1})" for a, b in bonded_pairs(sol, t, nA))
...
def check(sol, dzn=None):
    ...
    bonders = [
        ((sol["bonder_q1"][g], sol["bonder_r1"][g]), (sol["bonder_q2"][g], sol["bonder_r2"][g]))
        for g in range(len(sol.get("bonder_q1", [])))
    ]
    calcs = [(sol["calc_q"][g], sol["calc_r"][g]) for g in range(len(sol.get("calc_q", [])))]

    # reagent-input (spawn) pools -- asp/core2.lp semantics
    if dzn is not None:
        n_spawn = dzn.get("nSpawn", 0)
        spawn_hex = list(zip(dzn.get("spawn_q", []), dzn.get("spawn_r", []), strict=False))
    ...
    pred_of = [
        next(
            (
                b
                for b in range(nA)
                if atom_spawn[b] == atom_spawn[a] and atom_rank[b] == atom_rank[a] - 1
            ),
            None,
        )
        for a in range(nA)
    ...
    if dzn is not None:
        pos = list(zip(dzn["init_q"], dzn["init_r"], strict=False))
    ...
    if dzn is not None:
        ...
        pairs = (
            ...
            else [raw[i : i + 2] for i in range(0, len(raw), 2)]
        )
    ...
    for t in range(T + 1):
        ...
        for m in range(nM):
            ...
            assert g == (sol["gq"][t][m], sol["gr"][t][m]), f"t={t}: gripper mismatch arm {m}"
        for a in range(nA):
            ...
            if "ex" in sol:
                assert exist[a] == bool(sol["ex"][t][a]), f"t={t}: existence mismatch atom {a}"
            ...
            if held[a]:
                assert pos[a] == grip[held[a] - 1], f"t={t}: held atom {a} not on gripper"
        ...
        occ_spawn = [
            any(exist[b] and new_pos[b] == spawn_hex[s] for b in range(nA)) for s in range(n_spawn)
        ]
        exist = [
            exist[a]
            or (
                atom_rank[a] > 1
                and (p := pred_of[a]) is not None
                and exist[p]
    ...
    if dzn:
        ...
        slot_pairs = (
            raw
            if not raw or isinstance(raw[0], list)
            else [raw[i : i + 2] for i in range(0, len(raw), 2)]
        )
        req = set()
        ...
        if dzn.get("exact_molecule"):
            ...
            for a, b in bonds:
                if a in prodset or b in prodset:
                    assert (a, b) in req, f"goal: extra bond (a{a + 1},a{b + 1}) on the product"
...
def main():
    ...
    ap.add_argument("--check", action="store_true", help="re-simulate and verify the plan is legal")
    ...
    if args.check:
        ...
        n = check(sol, dzn)
        print(
            f"CHECK OK: legal plan, {n} actions"
