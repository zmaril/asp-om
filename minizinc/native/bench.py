...
def bench_native(inst, variant, threads):
    ...
    for _ in range(REPEATS):
        out, wall = run(
            [sys.executable, os.path.join(HERE, "native_cpsat.py"),
             f"instances/{inst}.dzn",
             f"--{variant}", "--threads", str(threads), "--quiet",
             "--timeout", str(TIMEOUT_S)], TIMEOUT_S)
        m = re.search(r"status=(\S+) objective=(\d+) build_s=([\d.]+) "
                      r"solve_s=([\d.]+)", out)
        assert m, out[-500:]
        outs.append(dict(status=m.group(1), obj=int(m.group(2)),
                         build=float(m.group(3)), solve=float(m.group(4)),
                         wall=wall))
    med = lambda k: statistics.median(o[k] for o in outs)
    assert len({o["obj"] for o in outs}) == 1
    return dict(status=outs[0]["status"], obj=outs[0]["obj"],
                build=med("build"), solve=med("solve"), wall=med("wall"))
...
def bench_mzn(inst, variant, threads):
    ...
    for _ in range(REPEATS):
        out, wall = run(
            ["minizinc", "--solver", "cp-sat", "-p", str(threads), "-s",
             "--time-limit", str(TIMEOUT_S * 1000),
             model, f"instances/{inst}.dzn"], TIMEOUT_S)
        flat = re.search(r"flatTime=([\d.]+)", out)
        solve = re.findall(r"solveTime=([\d.eE+-]+)", out)
        objs = re.findall(r"total_actions = (\d+)", out)
        status = "OPTIMAL" if "==========" in out else \
                 ("FEASIBLE" if objs else "UNKNOWN")
        outs.append(dict(status=status,
    ...
    assert len({o["obj"] for o in outs}) == 1
    return dict(status=outs[0]["status"], obj=outs[0]["obj"],
                flat=med("flat"), solve=med("solve"), wall=med("wall"))
...
def main():
    for inst in INSTANCES:
        for variant in VARIANTS:
            ...
            rows.append(dict(
                instance=inst, variant=variant, threads=1,
                native_status=nat["status"], native_obj=nat["obj"],
                native_build_s=round(nat["build"], 3),
                native_solve_s=round(nat["solve"], 3),
                native_wall_s=round(nat["wall"], 3),
                mzn_status=mzn["status"], mzn_obj=mzn["obj"],
    ...
    for inst, variant in [("t2_bond", "free")]:
        ...
        rows.append(dict(
            instance=inst, variant=variant, threads=8,
            native_status=nat["status"], native_obj=nat["obj"],
            native_build_s=round(nat["build"], 3),
            native_solve_s=round(nat["solve"], 3),
            native_wall_s=round(nat["wall"], 3),
