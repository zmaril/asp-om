...
def parse_dzn(path):
    ...
    for stmt in src.split(";"):
        ...
        if val in ("true", "false"):
            ...
            inner_m = re.search(r"\[(.*)\]", val, re.S)
            if inner_m is None:
                raise ValueError(f"unhandled array2d value for {name}: {val[:40]}")
            flat = [int(x) for x in inner_m.group(1).split(",") if x.strip()]
...
class OmModel:
    ...
    def and_reif(self, lits, tag=""):
        ...
        self.m.AddBoolOr([lit.Not() for lit in lits]).OnlyEnforceIf(b.Not())
        return b
    ...
    def or_reif(self, lits, tag=""):
        ...
        self.m.AddBoolAnd([lit.Not() for lit in lits]).OnlyEnforceIf(b.Not())
        return b
    ...
    def build(self, fixed_layout):
        ...
        part_cells = (
            [cell(base_q[k], base_r[k]) for k in ARM]
        ...
        for t in STEP:
            m.Add(sum(doCW[t]) + sum(doCCW[t]) + sum(doGrab[t]) + sum(doDrop[t]) <= 1)
        ...
        heldEq = [
            [[self.eq_reif(held[t][a], k + 1, f"he{t}_{a}_{k}") for k in ARM] for a in ATOM]
            for t in TIME
        ]
        held0 = [[self.eq_reif(held[t][a], 0, f"h0{t}_{a}") for a in ATOM] for t in TIME]
                ...
                for g in range(nB):
                    onb[t, a, g, 1] = self.and_reif(
                        [
                            ...
                            self.eq_reif(ar[t][a], bonder_r1[g]),
                        ],
                        f"on1_{t}_{a}_{g}",
                    )
                    onb[t, a, g, 2] = self.and_reif(
                        [
                            ...
                            self.eq_reif(ar[t][a], bonder_r2[g]),
                        ],
                        f"on2_{t}_{a}_{g}",
                    )
            ...
            for a in ATOM:
                for b in ATOM:
                    if a < b:
                        m.Add(
                            bonded[t + 1, a, b]
                            == self.or_reif([bonded[t, a, b], formed(t + 1, a, b)])
                        )
            ...
            for k in ARM:
                ...
                for _ in range(1, nA):
                    ...
                    for a in ATOM:
                        ...
                        for b in ATOM:
                            if b != a:
                                lits.append(self.and_reif([stage[b], bnd(t, a, b)]))
        ...
        rot = [[self.or_reif([doCW[t][k], doCCW[t][k]], f"rot{t}_{k}") for k in ARM] for t in STEP]
        ...
        grabA: list[list[list[Any]]] = [[[None] * nA for _ in ARM] for _ in STEP]
        for t in STEP:
            for k in ARM:
                for a in ATOM:
                    at_grip = self.and_reif(
                        [self.eq_reif(aq[t][a], gq[t][k]), self.eq_reif(ar[t][a], gr[t][k])],
                        f"ag{t}_{k}_{a}",
                    )
                    grabA[t][k][a] = self.and_reif([doGrab[t][k], at_grip], f"grabA{t}_{k}_{a}")
        for t in STEP:
            for k in ARM:
                ...
                m.AddBoolOr([heldEq[t][a][k] for a in ATOM]).OnlyEnforceIf(doDrop[t][k])

        dropA = [
            [
                self.or_reif(
                    [self.and_reif([doDrop[t][k], heldEq[t][a][k]]) for k in ARM], f"dropA{t}_{a}"
                )
                for a in ATOM
            ]
            for t in STEP
        ...
        for t in STEP:
            for a in ATOM:
                ...
                frame = [dropA[t][a].Not()] + [grabA[t][k][a].Not() for k in ARM]
        ...
        movedA = [
            [
                self.or_reif(
                    [self.and_reif([rot[t][k], comp[t][k][a]]) for k in ARM], f"moved{t}_{a}"
                )
                for a in ATOM
            ]
            for t in STEP
        ...
        for t in STEP:
            for k in ARM:
                for a in ATOM:
                    ...
                    m.Add(aq[t + 1][a] == base_q[k] + base_r[k] - ar[t][a]).OnlyEnforceIf(
                        [doCW[t][k], comp[t][k][a]]
                    )
                    m.Add(ar[t + 1][a] == aq[t][a] + ar[t][a] - base_q[k]).OnlyEnforceIf(
                        [doCW[t][k], comp[t][k][a]]
                    )
                    # ccw about (BQ,BR): (Q,R) -> (Q+R-BR, BQ+BR-Q)
                    m.Add(aq[t + 1][a] == aq[t][a] + ar[t][a] - base_r[k]).OnlyEnforceIf(
                        [doCCW[t][k], comp[t][k][a]]
                    )
                    m.Add(ar[t + 1][a] == base_q[k] + base_r[k] - aq[t][a]).OnlyEnforceIf(
                        [doCCW[t][k], comp[t][k][a]]
                    )
            for a in ATOM:
        ...
        for t in TIME:
            ...
            for a in ATOM:
                for k in ARM:
                    m.AddBoolOr(
                        [
                            self.eq_reif(aq[t][a], base_q[k]).Not(),
        ...
        for t in STEP:
            for a in ATOM:
                ...
                onc = self.or_reif(
                    [
                        self.and_reif(
                            [self.eq_reif(aq[t][a], calc_q[g]), self.eq_reif(ar[t][a], calc_r[g])]
                        )
                        for g in range(nC)
                    ],
                    f"onc{t}_{a}",
        ...
        if d["require_any_bond"]:
            m.AddBoolOr([bonded[T, a, b] for a in ATOM for b in ATOM if a < b])
        ...
        m.Add(
            total
            == sum(
                doCW[t][k] + doCCW[t][k] + doGrab[t][k] + doDrop[t][k] for t in STEP for k in ARM
            )
        )
        m.Minimize(total)
        ...
        acted = [
            self.or_reif(doCW[t] + doCCW[t] + doGrab[t] + doDrop[t], f"acted{t}") for t in STEP
        ]
        if nM == 1:
            for t in range(1, T):
...
def extract(model, solver):
    ...
    sol = {
        ...
        "bonded": [
            [
                [bool(val(v["bonded"][t, a, b])) if a < b else False for b in range(nA)]
                for a in range(nA)
            ]
            for t in range(T + 1)
        ],
...
def print_trace(sol):
    ...
    for g in range(len(sol["bonder_q1"])):
        print(
            f"bonder {g + 1}: ({sol['bonder_q1'][g]},{sol['bonder_r1'][g]})"
    ...
    for t in range(T + 1):
        arms = " ".join(
            f"arm{k + 1} dir={sol['ori'][t][k]} grip=({sol['gq'][t][k]},{sol['gr'][t][k]})"
            for k in range(nM)
        )
        atoms = " ".join(
...
def main():
    ...
    mode.add_argument(
        "--fixed", action="store_true", help="pin layout to given_* params (om_fixed.mzn)"
    )
    mode.add_argument(
        "--free", action="store_true", help="layout is a decision variable (om.mzn, default)"
    )
    ap.add_argument("--threads", type=int, default=1, help="CP-SAT num_workers (default 1)")
    ap.add_argument("--timeout", type=float, default=300.0, help="max solve seconds (default 300)")
    ap.add_argument("--json", metavar="FILE", help="dump the solution as minizinc-style JSON")
    ...
    if args.json:
        with open(args.json, "w") as f:
            json.dump(sol, f)
    print(
        f"status={sname} objective={sol['total_actions']} "
